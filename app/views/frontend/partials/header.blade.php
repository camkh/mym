...
$imgLogo = 0;
if(Config::get('app.logo')) {
	$imgLogo = Config::get('app.url') . 'upload/' . Config::get('app.logo');
}
?>
	...
	var aDs = {{$getConfig->getConfig('adsense_id_2')->value}};
	@else
	var aDs = {{$getConfig->getConfig('adsense_id')->value}};
	@endif
	</script>
</head>
            ...
            <div class="navbar">
                <div class="navbar-inner">
                    ...
                    <div class="brand-wrapper">
                        <a class="brand navbar-brand @if($imgLogo) imgLogo @endif" href="{{Config::get('app.url')}}" title="Home, Watch movie online free"><i class="icon-check-empty"></i>
                        @if(Config::get('app.logo')) 
                        <img alt="Home, Watch movie online free" src="{{$imgLogo}}"/>
                        @elseif(Config::get('app.url') == 'http://www.allkun.com/')
                         ALL<span>KUN</span>.COM
                    	@elseif(Config::get('app.url') == 'http://www.roeung.com/')
                    	ROEUNG<span>.COM</span>
                    	@elseif(Config::get('app.url') == 'http://www.sruol9.com/')
                    	SRUOL<span>9.COM</span>
                    	@elseif(Config::get('app.url') == 'http://www.khmermovies.co/')
                    	KHMER<span>MOIVES.COM</span>
                    	@elseif(Config::get('app.url') == 'http://www.moviekhmer.co/')
                    	MOVIE<span>KHMER.CO</span>
                    	@elseif(Config::get('app.url') == 'http://www.mkhmer.co/')
                    	M<span>KHMER.CO</span>
                    	@elseif(Config::get('app.url') == 'http://www.m-khmer.com/')
                    	M-<span>KHMER.COM</span>
                    	@else
                    	<img alt="" src="{{$imgLogo}}"/>
                    	@endif
                    ...
                    <ul class="nav navbar-nav navbar-right">
						...
						<li>
							<a class='img-icon' href='http://www.roeung.com/' target='_blank' title="more movie">
								<svg class='icon icon-film'><use xlink:href='#icon-film'/></svg>
							</a>
						</li>
						<li>
							<a class='img-icon' href='http://www.topkhmersong.com/' target='_blank' title="Music online">
								<svg class='icon icon-headphones'><use xlink:href='#icon-headphones'/></svg>
							</a>
						</li>
															<!--Menu list item-->
						<li>
							<a class='img-icon' href='http://www.wwitv.co/' target='_blank' title="Watch TV online">
								<svg class='icon icon-tv'><use xlink:href='#icon-tv'/></svg>
							</a>
						</li>

						<!--Menu list item-->
						<li>
							<a class='img-icon' href='http://www.khvcd.com/' target='_blank' title="Music Video">
						...
						<!--Menu list item-->
						<li>
							<a class='img-icon' href='http://www.sronuk.com/' target='_blank' title="Daily News">
								<svg class='icon icon-newspaper'><use xlink:href='#icon-newspaper'/></svg>
							</a>
						</li>

						<!--Menu list item-->
						<li>
							<a class='img-icon' href='http://www.khmerdl.info/' target='_blank' title="Download">
								<svg class='icon icon-box-add'><use xlink:href='#icon-box-add'/></svg>
							</a>
						</li>
						<li>
							<a class='img-icon' id="search-bar" href='#' title="Download" style="margin-top: 5px">

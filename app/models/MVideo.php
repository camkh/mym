...
class MVideo extends Eloquent {
	 ...
	 * @author Socheat
	 */
	public function getVideo($id = null, $whereArr = array()) {
	...
	public function getVideoLists($id = null, $whereArr = array()) {
		...
		try {
			...
			$result = DB::table ( Config::get ( 'constants.TABLE_NAME.VIDEO' ) )
			->select ( '*' )
			->where ( $byWhere )
			->orderBy ( 'added', 'DESC' )
			->paginate ( $this->getConfig ( 'browse_page' )->value );
			$getVideos = $this->getVideoList ( $result );
	 ...
	 * @author Socheat
	 */
	public function getVideoListsByTag($cateID = null, $whereArr = array()) {
		$response = new stdClass ();
		try {
			if(!empty($cateID)) {
				$whereParent = array(
				...
				$getAllCateIn = $this->getCategory(null,$whereParent);
				$Cat_id = array();
				...
				$query = DB::table ( Config::get ( 'constants.TABLE_NAME.VIDEO' ) );
				$query->leftJoin('videos_cat_relationships as re', 'videos.uniq_id', '=', 're.object_id');
				$query->whereIn('re.vdo_categories', array($cat_id));
				$query->orderBy ( 'added', 'DESC' );
	...
	public function getVideoList($param) {
		...
		try {
			foreach ( $param as $vdo ) {
				...
				// $uniq_id, $video_title = '', $url_type = '', $tositeb = ''
				
				$list [$i] ['id'] = $vdo->id;
				$list [$i] ['link'] = $this->makeVideoLink ( $vdo->uniq_id, $vdo->video_title, null, $vdo->to_site_b );
				$list [$i] ['thumb'] = $this->resizeImage ( $vdo->thumb, 'w250-h200-p' );
				$list [$i] ['image'] = $this->resizeImage ( $vdo->thumb, 's0' );
				$list [$i] ['title'] = preg_replace ( '/[\x00-\x1F\x80-\xFF]/', '', $tiles );
				$list [$i] ['views'] = $this->vdo_compact_number_format ( $vdo->site_views );
				$list [$i] ['tositeb'] = $vdo->to_site_b;
				$list [$i] ['total'] = json_decode ( $vdo->playlist )->total_list;
			}
			return $list;
		} catch ( Exception $e ) {
			return array (
	...
	public function getVideoDataList($param) {
		...
		try {
			foreach ( $param as $nVideo ) {
				...
				$dataList .= '<span class="pm-label-duration border-radius3 opac7">' . $nVideo ['total'] . ' Parts</span>';
				$dataList .= '</span>';
				if ($nVideo ['tositeb'] == 1) {
					$target = "_blank";
				} else {
				...
				$dataList .= '<a href="' . $nVideo ['link'] . '" target="' . $target . '" class="pm-thumb-fix pm-thumb-145" title="' . $nVideo ['title'] . '">';
				$dataList .= '<span class="pm-thumb-fix-clip">';
				$dataList .= '<img src="' . $nVideo ['thumb'] . '" width="145"/>';
				...
				$dataList .= '</div>';
				$dataList .= '<h3 dir="ltr">';
				$dataList .= '<a href="' . $nVideo ['link'] . '" target="' . $target . '" class="pm-title-link" title="' . $nVideo ['title'] . '">';
				$dataList .= $this->rm($nVideo ['title'],45);
				$dataList .= '</a>';
				$dataList .= '</h3>';
	...
	public function getConfig($name = '') {
		...
		if (! empty ( $name )) {
			...
			$result = DB::table ( 'videos_config' )->where ( $byWhere )->first ();
		} else {
			$result = DB::table ( 'videos_config' )->get ();
		}
		return $result;
	}
	public function makeVideoLink($uniq_id, $video_title = '', $url_type = '', $tositeb = '') {
		$video_title_clean = $this->sanitize_title ( $video_title );
		$new_url = Config::get ( 'app.url' );
		if(Config::get('app.siteNoAd')==1) {
			$new_url = $this->getConfig ( 'url_watch' )->value;
			if ($tositeb) {
				$new_url = $this->getConfig ( 'url_ad1' )->value;
			}
		} else if ($tositeb) {
			if(!Config::get('app.toSite')==1) {
				$new_url = $this->getConfig ( 'url_ad1' )->value;
			}
		} else if(Config::get('app.watchTo')==1) {
			$new_url = $this->getConfig ( 'url_watch' )->value;
		}
		return $new_url . $video_title_clean . '-video_' . $uniq_id . '.html';
	}
	...
	public function getCategory($id = array(), $byWhere = array(), $whereGetFirst=array()) {
		$query = DB::table ( 'videos_categories' );
		$query->select ( '*' );
		if(!empty($byWhere)) {
			$query->where ( $byWhere );
		}
	...
	public function getRelatedVideoList($category_id) {
		...
		$list = $this->queryVideoList($result);
    	return $list;
	}
